...
<!DOCTYPE html>
<html><head>
 <?$t="Connections - Manage Account";$OP->inc("inc/head.php");?>
</head><body>
 ...
 <div class="content">
  ...
  <h2>Following</h2>
  <?
  function parse_$OP->followButton($id){
   $img=get("img", $id);
   ...
   $h.="<div style='position:absolute;top:0px;right:0px;text-align:left;'>".$OP->followButton($id)."</div>";
  ...
  if($sql->rowCount()==0){
   if($q==''){$OP->sss("You're Lonely", "Friends make everything better. Follow Some Persons to enjoy <b>Open</b> more.");exit;}
  }
  echo"<center>";
  while($r=$sql->fetch()){  
   parse_$OP->followButton($r['id']);
  }
  echo"</center>";
  ?>
  <h2>Followers</h2>
  ...
  $sql->execute(array(":who"=>$who));
  if($sql->rowCount()==0){
   if($q==''){$OP->sss("No One Found", "No one is following you. Sorry.");exit;}
  }
  echo"<center>";
  while($r=$sql->fetch()){  
   parse_$OP->followButton($r['id']);
