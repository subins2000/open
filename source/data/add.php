 	...
 	if(is_numeric($uid) && is_array($FILE) && count($FILE) > 0){
  		...
  		if(array_search($extension, $extensions)!==false){
   		...
   		if($resize->imgw() > 500 || $resize->imgh() > 500){
    			...
    			$newWidth  = $resize->imgw()/$total*700;
    			$newHeight = $resize->imgh()/$total*700;
   		}else{
   		...
   		$sql=$OP->dbh->prepare("SELECT `uid` FROM `data` WHERE `uid`=? AND `name`=?");
   		...
   		return HOST . "/data/$uid/$uploadName";
  		}else{
