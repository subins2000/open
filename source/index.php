...
<html>
 ...
 <div style="width:100%;background:white;color:white;text-align:center;">
  <div class="icontent">
   ...
   <div style="font-size: 30px;margin-top: 25px;height: 25px;">
    <?php
    $sql=$OP->dbh->prepare("SELECT COUNT(`id`) FROM `users`");
    $sql->execute();
    $count=$sql->fetchColumn();
    foreach(str_split($count) as $v){
      echo "<span style='background:black;padding: 4px 15px;border-right: 1px solid white;'>$v</span>";
    }
    ?>
   </div>
   <div style="background: rgb(100, 172, 255);width: 200px;margin: 10px auto 0px;padding: 5px 30px;">Users So Far</div>
  </div>
 </div>
