...
if(!isset($_GET['id']) && !$OP->lg){
 $OP->init(true);
}
if($_SERVER['SCRIPT_NAME']!="/index.php" && $id!=$who){
 $OP->$OP->redirect(HOST . "/$id");
}
...
while($r=$sql->fetch()){
 ...
 $img=$img=='' ? HOST . "/source/cdn/img/profile_pics/om":$img;
 ...
 $himg=$json['header'];
 $himg=$himg=="" ? HOST . "/source/cdn/img/headers/00.png":$himg;
}
$pvals=array($about, $bir);
require("$docRoot/inc/class.rep.php");
$RP=new ORep();
$Rep=$RP->getRep($id);
?>
<!DOCTYPE html>
  ...
  <div class="main blocks">
   <div class="clearfix left block">
    ...
    <div class="noggler" hide id="feed" <?if($_GET['part']==""){echo"show";}?>>
     <?$_POST['user']=$id;$OP->inc("inc/feed.php");?>
    </div>
     ...
     <div class="blocks">
      <div class="block" style="width: 49%;">
       ...
       ?>
        <div class="blocks item">
         ...
         <a class="block" href="<?echo HOST;?>/view/<?echo$r['id'];?>">Post # <?echo$r['id'];?></a>
        </div>
       <?
       }
       ?>
      ...
      <div class="block" style="width: 49%;">
       ...
       ?>
        <div class="blocks item">
         <div class="block rep"><?echo$r['rep'];?></div>
         <a class="block" href="<?echo HOST;?>/view/<?echo$r['pid']."#".$r['id'];?>">
          <?
          $c=$r['cmt'];
          if(strlen($c) > 10){
           echo substr($c, 0, 10);
   ...
   <div class="clearfix right block">
    ...
    <div class="osin">
     ...
     <div class="following" style="text-align:center;">
      ...
      $sql=$OP->dbh->prepare("SELECT COUNT(uid) FROM conn WHERE uid=?");
      $sql->execute(array($id));
      $sql=$sql->fetchColumn();
     ...
     <div class="followers" style="text-align:center;">
      ...
      $sql=$OP->dbh->prepare("SELECT 1 FROM conn WHERE fid=?");
      $sql->execute(array($id));
