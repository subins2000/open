		...
		<div class="wrapper"><div class="content" style="text-align:center;">
			...
			<div id="classic">
				...
				if(isset($_POST['verify']) && $code!=""){
					...
					$origSTR = "{$_POST['user']}OverifyCode985";
					if($origSTR == $decoded){
						$verified = 1;
					}else{
						$verified = 0;
				...
				if( isset($_POST['verify']) && !isset($_POST['code']) ){
					...
					$secretToken = $OP->encrypt("{$email}OverifyCode985");
					echo $secretToken;
					$OP->sendEMail($email, "Verify Your E-Mail", "You requested for registering on Open. For signing up, you need to verify your E-Mail address. Paste the code below in the input field of the page where you requested for signing up.<blockquote>{$secretToken}</blockquote>");
				?>
				...
				if(isset($_POST['submit'])){
					...
					$origSTR = "{$_POST['nuser']}OverifyCode985";
					if($origSTR != $decoded){
