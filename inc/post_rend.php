  ...
  $h.="<div class='post blocks' id='$id'>";
   ...
   $h.="<div class='right block'>";
    $h.="<div class='top'>";
     ...
     $h.="<a class='time slink' href='" . HOST . "/view/{$id}'>{$v['time']}</a>";
     $h.="<span class='slink' title='{$prd}'>{$prs}</span>";
     if($v['uid']==$who){
      $h.="<div class='author_box'><div class='author_panel c_c'><a class='de_post pointer' id='$id'>Delete Post</a><br/><cl/><a class='pointer' onclick='dialog(\"<h2>Post Link</h2><textarea style=\\\"width: 50%;\\\">" . HOST . "/view/{$id}</textarea>\", true)' id='$id'>Get Link</a></div></div>";
     }
    $h.="</div>";
    $h.="<div class='cont'>";
     $h.="$p";
