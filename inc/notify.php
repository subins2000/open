...
if(!function_exists("notify")){
 function notify($action, $t, $pid, $to, $w){
  ...
  if($action=="comment" && $to!=$who){
   ...
   $cCmts=$sql->rowCount();
   $m=$n." commented on your <a href='http://open.subinsb.com/view/$pid'>post</a> :";
   $m.="<blockquote>$t</blockquote>";
   $m.="Your post now have <b>$cCmts</b> comments.<br/><a href='http://open.subinsb.com/view/$pid#$lci' target='_blank'><button style='padding:5px 15px;'>View Post</button></a>";
  ...
  if($action=="mention" && $to!=$who){
   $sql=$db->prepare("DELETE FROM notify WHERE uid=? AND fid=? AND ty=?"); /* Delete Existing This Action (mention) Notifications */
   $sql->execute(array($to, $who, $t=="post" ? "men":"menc"));
   $sql=$db->prepare("INSERT INTO notify (uid, fid, ty, post, posted) VALUES (?, ?, ?, ?, NOW())"); /* And Add It again */
   if($t=="post"){
    $sql->execute(array($to, $who, "men", "0-$pid"));
   }else{
    $commentId=$db->prepare("SELECT id FROM cmt WHERE `pid`=? AND `uid`=? ORDER BY id DESC LIMIT 1");
   ...
   if($t=="post"){
    $m.="<a href='http://open.subinsb.com/view/$pid' target='_blank'>";
   }else{
    $m.="<a href='http://open.subinsb.com/view/$pid#$commentId' target='_blank'>";
   }
  ...
  if($action=="msg" && $to!=$who){
   ...
   if(strtotime($lps) < strtotime("-20 minutes") || $lps==""){
    ...
    $m.="<a href='http://open.subinsb.com/chat/$w'><button style='padding:5px 15px;'>See $sn's Messages</button></a>&nbsp;&nbsp;&nbsp;";
