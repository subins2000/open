...
if($numUsers == 0){
 	...
 	if($numUsers != 0){
 		$sql = $OP->dbh->prepare("SELECT `id` FROM `users` WHERE `id`!=:who LIMIT 5 OFFSET ".(rand(0, $numUsers)).")");
 		$sql->execute(array(":who" => $who));
 		
 	}
}
if($sql->rowCount()!=0){
 	echo '<div class="suggestions" style="padding: 5px 0px;margin: 5px -10px 0px -15px;color: black;">';
 		echo "<h2>Suggestions</h2>";
 		while($r=$sql->fetch()){
  			$userId     = $r['id'];
  			$name 		= get("name", $u,false);
  			$firstName  = get("fname", $u,false);
  			...
  			echo "<div class='sugg'>";
   			...
   			echo "</div>";
   			echo "<div style='vertical-align:top;display:inline-block;'>";
    				echo "<a title='$nm' href='$plink' style='padding-left:5px;'>$firstName</a><br/><cl/>";
